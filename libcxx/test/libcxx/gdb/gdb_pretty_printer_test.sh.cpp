...
void streampos_test() {
  ...
  ComparePrettyPrintToRegex(
      test0,
      R"(std::fpos with stream offset:67(with state: {count:0 value:0})?)");
  std::istringstream input("testing the input stream here");
  std::streampos test1 = input.tellg();
  ComparePrettyPrintToRegex(
      test1,
      R"(std::fpos with stream offset:0( with state: {count:0 value:0})?)");
  std::unique_ptr<char[]> buffer(new char[5]);
  input.read(buffer.get(), 5);
  test1 = input.tellg();
  ComparePrettyPrintToRegex(
      test1,
      R"(std::fpos with stream offset:5( with state: {count:0 value:0})?)");
}
